# Audit Report: Doreen Booth Profile — Accessibility (WCAG 2.1 AA)
...
## 1. Executive Summary
The single team member profile for Doreen Booth provides a well-structured biographical presentation with a semantic `<h1 class="entry-title">Doreen Booth</h1>` heading and a populated alt attribute on her featured portrait (`alt="Doreen Booth"`). The key accessibility finding is the use of multiple consecutive `<br><br><br><br>` tags within a single Kadence advanced heading paragraph rather than semantic `<p>` paragraph elements, which can cause screen readers to announce multiple empty lines.
...
| 2 | Non-text Content (Alt Text) | Featured Headshot Image | **Low** | The portrait image `<img ... alt="Doreen Booth">` has valid alt text. To enhance assistive context, adding her role is recommended. | Update alt attribute to `alt="Doreen Booth - Project Manager"`. |
...
- [x] **H1 Heading Presence:** `<h1 class="entry-title">Doreen Booth</h1>` is present.
- [x] **Alt Attribute Completeness:** Featured portrait has `alt="Doreen Booth"`.
...
1. **Enrich Portrait Alt Text:** Set `alt="Doreen Booth - Project Manager"` in the Media Library.
2. **Bio Role Subtitle:** Add a semantic role subtitle `<p class="team-member-role">Project Manager</p>` beneath the H1.
