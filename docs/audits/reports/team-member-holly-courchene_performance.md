# Audit Report: Holly Courchene (Holly Spence) Profile — Performance & Runtime

**Page URL:** `https://indigenoustourismmanitoba.ca/team-member/holly-courchene/`  
**Audit Concern:** `performance`  
**Date Audited:** `2026-08-31`  
**Runtime Health:** `Good (2.14 MB Total Image Payload; 225.8 KB Scaled Headshot Portrait; Clean Console)`  

---

## 1. Executive Summary
The single team member profile for Holly Courchene (Holly Spence) demonstrates clean runtime script execution with zero JavaScript console errors. The primary performance optimization opportunity is serving a responsive, WebP-compressed variant of the featured portrait (`Holly-Spence-headshot-2025-scaled.jpg` transfers 225.8 KB for an unscaled 2560x1760 JPEG), eliminating redundant Bootstrap Icons stylesheets, and providing explicit dimensions for global card/header imagery to prevent layout shifts.
...
|---|---|---|---|---|---|
| 1 | Image Optimization | `Holly-Spence-headshot-2025-scaled.jpg` | **Medium** | The profile headshot is served as a 2560x1760 JPEG transferring **225.8 KB** for a ~350px display container. | Serve a registered `medium_large` WebP crop (~400px width; < 35 KB; 84% reduction). |
| 2 | Total Image Payload | Page-Wide Media Payload | **Medium** | Combined image payload is **2.14 MB** across 7 assets. | Implement WebP conversion and responsive `srcset` delivery across featured and header assets. |
...
- [ ] **Image Compression & Sizing:** Headshot transfers 225.8 KB (unscaled 2560px JPEG); total image weight is 2.14 MB.
