# Audit Report: Accommodation Category Archive — Accessibility (WCAG 2.1 AA)
...
## 1. Executive Summary
The Accommodation operator category taxonomy archive lists Indigenous-owned lodging options in Manitoba (Wyndham Garden Winnipeg Airport, Turtle Village, Moon Gate Guest House, Kikiwak Inn). While card images have proper descriptive alt text and links are keyboard focusable, the page **completely lacks an `<h1>` heading**, creating a major accessibility barrier for assistive technology users navigating taxonomy archives.
...
| 3 | Non-text Content (Alt Text) | Listing Card Thumbnails | **Low** | All 4 accommodation card images have clean alt attributes matching the operator business names. | Good baseline compliance. |
...
- [x] **Alt Attribute Presence:** 4 of 4 accommodation operator cards have populated alt attributes.
...
1. **Add Results Counter:** Include an `aria-live="polite"` result count (e.g. "Showing 4 Indigenous accommodation operators in Manitoba").
