# Audit Report: Doreen Booth Profile — Performance & Runtime

**Page URL:** `https://indigenoustourismmanitoba.ca/team-member/doreen-booth/`  
**Audit Concern:** `performance`  
**Date Audited:** `2026-08-31`  
**Runtime Health:** `Good (2.16 MB Total Image Payload; 244.4 KB Scaled Headshot Portrait; Clean Console)`  

---

## 1. Executive Summary
The single team member profile for Doreen Booth executes cleanly with zero JavaScript console errors or runtime failures. The primary optimization opportunities include serving modern WebP image formats for the featured portrait (`Doreen-Booth-HS1-scaled.jpg` transfers 244.4 KB for a 2560x1706 image), eliminating redundant Bootstrap Icons stylesheets, and providing explicit dimensions for global card/header imagery to prevent layout shifts.
...
|---|---|---|---|---|---|
| 1 | Image Optimization | `Doreen-Booth-HS1-scaled.jpg` | **Medium** | The profile headshot is served as a 2560x1706 JPEG transferring **244.4 KB** for a ~350px display container. | Serve a registered `medium_large` WebP crop (~400px width; < 35 KB; 85% reduction). |
| 2 | Total Image Payload | Page-Wide Media Payload | **Medium** | Combined image payload is **2.16 MB** across 7 assets. | Implement WebP conversion and responsive `srcset` delivery across featured and header assets. |
...
- [ ] **Image Compression & Sizing:** Headshot transfers 244.4 KB (unscaled 2560px JPEG); total image weight is 2.16 MB.
