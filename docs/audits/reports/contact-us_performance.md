# Audit Report: Contact Us — Performance & Runtime

**Page URL:** `https://indigenoustourismmanitoba.ca/contact-us/`  
**Audit Concern:** `performance`  
**Date Audited:** `2026-08-30`  
**Runtime Health:** `Good (Deferred Form Assets; Dimension Attributes Needed)`  

---

## 1. Executive Summary
The Contact Us page loads efficiently with an overall lightweight footprint (< 2.0 MB) and zero runtime JavaScript exceptions. While Gravity Forms enqueues multiple theme stylesheets and utility scripts, all non-critical form assets are correctly configured with `defer="defer"`, allowing the initial DOM to paint smoothly. The primary performance enhancements involve declaring explicit `width` and `height` attributes on top feature cards and eliminating redundant Bootstrap Icons stylesheets.
...
| 3 | Form Stylesheet Concatenation | Gravity Forms Orbital CSS | **Low** | Gravity Forms enqueues 4 separate orbital/framework stylesheets. | Enable CSS combination/minification via caching plugin. |
...
- [x] **Form Script Deferral:** Gravity Forms scripts correctly load with `defer=True`.
