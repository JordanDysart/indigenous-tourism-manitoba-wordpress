# Audit Report: North Region Archive — Accessibility (WCAG 2.1 AA)

**Page URL:** `https://indigenoustourismmanitoba.ca/operator-region/north/`  
**Audit Concern:** `accessibility`  
**Date Audited:** `2026-08-31`  
**Overall Status:** `Needs Remediation (Missing H1 Region Heading; 100% Populated Card Alt Text across 3 Northern Operators)`  

---

## 1. Executive Summary
The North region operator taxonomy archive indexes 3 premier Indigenous tourism operators across Northern Manitoba and the sub-Arctic Hudson Bay coastline: Kikiwak Inn (First Nations-owned hotel, conference center, and cultural hospitality in Opaskwayak Cree Nation / The Pas), Wapusk Adventures (world-renowned dogsledding expeditions and Northern Lights tours in Churchill), and Sub-Arctic Tours (custom tundra, beluga whale, and polar bear excursions in Churchill). All 3 listing card thumbnails feature clean, descriptive alt text matching business names, and keyboard navigation flows logically. However, the archive **completely lacks an `<h1>` heading**, creating a critical navigation barrier for screen reader users browsing regional directories.
...
1. **Add Results Counter:** Include an `aria-live="polite"` result count (e.g. "Showing 3 Indigenous operators in the Northern region").
