# Audit Report: Tour and Related Services Category Archive — Accessibility (WCAG 2.1 AA)
...
## 1. Executive Summary
The Tour and Related Services operator category archive indexes cultural guided tours and Treaty education centres across Manitoba (featuring Whiteshell Petroforms Authentic Indigenous Tours and Agowiidiwinan Centre). The listing card images feature clean descriptive alt text matching business names, and the keyboard navigation sequence is logical. However, the archive **completely lacks an `<h1>` heading**, creating a critical navigation barrier for screen reader users browsing directory categories.
...
| 3 | Non-text Content (Alt Text) | Listing Card Thumbnails | **Low** | Both card images have clean alt attributes matching the operator business names (Whiteshell Petroforms, Agowiidiwinan Centre). | Good baseline compliance. |
...
- [x] **Alt Attribute Presence:** 2 of 2 tour operator cards have populated alt attributes.
...
1. **Add Results Counter:** Include an `aria-live="polite"` result count (e.g. "Showing 2 Indigenous tour and related service operators in Manitoba").
